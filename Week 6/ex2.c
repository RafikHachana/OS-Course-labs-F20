...
int main()
{
	...
	int* wt;
	int* tat;
	printf("----   Welcome to the Shortest job first algorithm ----\n");
	...
	//sort according to arrival time first, then according to burst time
	bool done = false;
	while(!done)
	{
		done = true;
	...
	int time = 0;
	for(int i=0;i<n;i++)   //run processes in the sorted order (by burst time)
	{
